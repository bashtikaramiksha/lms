...
import { updateLessonSchema } from "@/lib/validations/curriculum";
...
export async function PATCH(
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: "Failed to update lesson" } },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const { id: lessonId } = await params;
    await courseService.deleteLesson(lessonId, session.user.id, role);
    ...
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: "Failed to delete lesson" } },
      { status: 500 }
    );
  }
}
