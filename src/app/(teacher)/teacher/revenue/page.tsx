...
import { TeacherRevenuePage } from "@/components/teacher/TeacherRevenuePage";

export default async function TeacherRevenue() {
  const session = await auth();

  if (!session?.user) {
    redirect("/login?callbackUrl=/teacher/revenue");
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <nav className="flex items-center gap-3 text-xs font-semibold">
              ...
              <Link href="/teacher/revenue" className="text-indigo-400 font-bold">
                Revenue
              </Link>
