...
import { LiveSessionsPage } from "@/components/teacher/live/LiveSessionsPage";
import { NotificationBell } from "@/components/shared/NotificationBell";

export default async function TeacherLiveSessions() {
  const session = await auth();

  if (!session?.user) {
    redirect("/login?callbackUrl=/teacher/live-sessions");
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <nav className="flex items-center gap-3 text-xs font-semibold">
              ...
              <Link href="/teacher/live-sessions" className="text-indigo-400 font-bold">
                Live Classes
              </Link>
